import React from "react";
import { Globe, Link as LinkIcon } from "lucide-react";

export type SocialPlatformId =
  | "instagram"
  | "youtube"
  | "tiktok"
  | "facebook"
  | "x"
  | "twitch"
  | "linkedin"
  | "snapchat"
  | "pinterest"
  | "other";

export interface PlatformConfig {
  id: SocialPlatformId;
  name: string;
  placeholder: string;
  defaultInputType: "username" | "url";
  urlPrefix: string;
  color: string;
  bgLight: string;
  borderClass: string;
  urlPattern?: RegExp;
}

export const PLATFORM_CONFIGS: Record<SocialPlatformId, PlatformConfig> = {
  instagram: {
    id: "instagram",
    name: "Instagram",
    placeholder: "username (e.g. johncreator)",
    defaultInputType: "username",
    urlPrefix: "https://instagram.com/",
    color: "text-pink-600 dark:text-pink-400",
    bgLight: "bg-pink-50 dark:bg-pink-950/40",
    borderClass: "border-pink-200 dark:border-pink-900/50",
    urlPattern: /^(https?:\/\/)?(www\.)?instagram\.com\/[a-zA-Z0-9_.-]+\/?$/i,
  },
  youtube: {
    id: "youtube",
    name: "YouTube",
    placeholder: "https://youtube.com/@username or channel URL",
    defaultInputType: "url",
    urlPrefix: "https://youtube.com/@",
    color: "text-red-600 dark:text-red-400",
    bgLight: "bg-red-50 dark:bg-red-950/40",
    borderClass: "border-red-200 dark:border-red-900/50",
    urlPattern: /^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be)\/.+/i,
  },
  tiktok: {
    id: "tiktok",
    name: "TikTok",
    placeholder: "username (e.g. johncreator)",
    defaultInputType: "username",
    urlPrefix: "https://tiktok.com/@",
    color: "text-neutral-900 dark:text-neutral-100",
    bgLight: "bg-neutral-100 dark:bg-neutral-900/60",
    borderClass: "border-neutral-200 dark:border-neutral-800",
    urlPattern: /^(https?:\/\/)?(www\.)?tiktok\.com\/@[a-zA-Z0-9_.-]+\/?$/i,
  },
  facebook: {
    id: "facebook",
    name: "Facebook",
    placeholder: "https://facebook.com/username or page URL",
    defaultInputType: "url",
    urlPrefix: "https://facebook.com/",
    color: "text-blue-600 dark:text-blue-400",
    bgLight: "bg-blue-50 dark:bg-blue-950/40",
    borderClass: "border-blue-200 dark:border-blue-900/50",
    urlPattern: /^(https?:\/\/)?(www\.)?facebook\.com\/.+/i,
  },
  x: {
    id: "x",
    name: "X (Twitter)",
    placeholder: "username (e.g. johncreator)",
    defaultInputType: "username",
    urlPrefix: "https://x.com/",
    color: "text-neutral-900 dark:text-neutral-100",
    bgLight: "bg-slate-100 dark:bg-slate-900/60",
    borderClass: "border-slate-200 dark:border-slate-800",
    urlPattern: /^(https?:\/\/)?(www\.)?(x\.com|twitter\.com)\/[a-zA-Z0-9_]+\/?$/i,
  },
  twitch: {
    id: "twitch",
    name: "Twitch",
    placeholder: "https://twitch.tv/username or handle",
    defaultInputType: "url",
    urlPrefix: "https://twitch.tv/",
    color: "text-purple-600 dark:text-purple-400",
    bgLight: "bg-purple-50 dark:bg-purple-950/40",
    borderClass: "border-purple-200 dark:border-purple-900/50",
    urlPattern: /^(https?:\/\/)?(www\.)?twitch\.tv\/[a-zA-Z0-9_]+\/?$/i,
  },
  linkedin: {
    id: "linkedin",
    name: "LinkedIn",
    placeholder: "https://linkedin.com/in/username",
    defaultInputType: "url",
    urlPrefix: "https://linkedin.com/in/",
    color: "text-sky-700 dark:text-sky-400",
    bgLight: "bg-sky-50 dark:bg-sky-950/40",
    borderClass: "border-sky-200 dark:border-sky-900/50",
    urlPattern: /^(https?:\/\/)?(www\.)?linkedin\.com\/(in|company)\/.+/i,
  },
  snapchat: {
    id: "snapchat",
    name: "Snapchat",
    placeholder: "username (e.g. johncreator)",
    defaultInputType: "username",
    urlPrefix: "https://snapchat.com/add/",
    color: "text-amber-500 dark:text-amber-400",
    bgLight: "bg-amber-50 dark:bg-amber-950/40",
    borderClass: "border-amber-200 dark:border-amber-900/50",
    urlPattern: /^(https?:\/\/)?(www\.)?snapchat\.com\/add\/[a-zA-Z0-9_.-]+\/?$/i,
  },
  pinterest: {
    id: "pinterest",
    name: "Pinterest",
    placeholder: "username (e.g. johncreator)",
    defaultInputType: "username",
    urlPrefix: "https://pinterest.com/",
    color: "text-red-700 dark:text-red-400",
    bgLight: "bg-rose-50 dark:bg-rose-950/40",
    borderClass: "border-rose-200 dark:border-rose-900/50",
    urlPattern: /^(https?:\/\/)?(www\.)?pinterest\.com\/[a-zA-Z0-9_.-]+\/?$/i,
  },
  other: {
    id: "other",
    name: "Other",
    placeholder: "Profile URL (e.g. https://mywebsite.com)",
    defaultInputType: "url",
    urlPrefix: "https://",
    color: "text-teal-600 dark:text-teal-400",
    bgLight: "bg-teal-50 dark:bg-teal-950/40",
    borderClass: "border-teal-200 dark:border-teal-900/50",
  },
};

export function SocialIcon({ platform, className = "w-5 h-5" }: { platform: string; className?: string }) {
  const p = platform.toLowerCase() as SocialPlatformId;

  switch (p) {
    case "instagram":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
        </svg>
      );
    case "youtube":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
        </svg>
      );
    case "tiktok":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.96-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.82.56-1.36 1.52-1.43 2.51-.08 1.12.39 2.27 1.25 2.98.9.75 2.18.96 3.27.56 1.05-.38 1.87-1.36 2.05-2.47.07-.63.05-1.27.05-1.91l-.01-13.48z"/>
        </svg>
      );
    case "facebook":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
        </svg>
      );
    case "x":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
        </svg>
      );
    case "twitch":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M11.571 4.714h1.715v5.143h-1.715zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714z"/>
        </svg>
      );
    case "linkedin":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
        </svg>
      );
    case "snapchat":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M12.003 1.13c-4.428 0-7.394 3.09-7.394 6.942 0 1.93.81 4.103 2.11 5.372.164.16.27.371.21.602-.12.459-.95 3.328-1.07 3.758-.08.29.04.57.309.68 1.05.43 2.5.58 3.51.15.429-.18.84.09 1.16.42 1.03.94 1.25 1.25 1.25 1.25.04.04.09.07.14.09.07.03.15.05.23.05.08 0 .16-.02.23-.05.05-.02.1-.05.14-.09 0 0 .22-.31 1.25-1.25.32-.33.731-.6 1.16-.42 1.01.43 2.46.28 3.51-.15.27-.11.39-.39.31-.68-.12-.43-.95-3.299-1.07-3.758-.06-.231.046-.442.21-.602 1.3-1.269 2.11-3.442 2.11-5.372 0-3.852-2.966-6.942-7.394-6.942z"/>
        </svg>
      );
    case "pinterest":
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.194 0 1.031.397 2.138.893 2.738.098.119.112.224.083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.631-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24 12.017 24c6.627 0 11.999-5.373 11.999-11.987C24.016 5.367 18.644 0 12.017 0z"/>
        </svg>
      );
    default:
      return <Globe className={className} />;
  }
}
